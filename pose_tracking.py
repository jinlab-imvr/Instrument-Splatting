...
def get_gripper_tips(instrument: Instrument, gripper_segmentation_mask, 
                     wrist_segmentation_mask, with_erosion = True):
    """
...
def track(instrument: Instrument, pose: Pose, viewpoint_cam, opt, pipe, background, global_iteration, exp_path):
    ...
    if gt_gripper_mask.sum() > 10:
        tips1_coord, tips2_coord = get_gripper_tips(instrument, 
                        ...
                        (gt_wrist_mask.detach().cpu()).numpy(), with_erosion = False )
        tips1_coord_erosion, tips2_coord_erosion = get_gripper_tips(instrument, 
                        (gt_gripper_mask.detach().cpu()).numpy(), 
                        (gt_wrist_mask.detach().cpu()).numpy(), with_erosion=True)
    ...
    for iteration in range(first_iter, tracking_iterations):
        ...
        if tips1_coord is not None:
            tips1_coord = torch.tensor(tips1_coord).cuda()
            tips2_coord = torch.tensor(tips2_coord).cuda()
        ...
        with torch.no_grad():
            # diff = torch.abs(image - gt_semantic_map)
    ...
    return pose_new, render_semantics.detach(), render_depth.detach(),\
...
def training(gs_type, dataset, opt, pipe, checkpoint, exp_name):
    ...
    exp_path = os.path.join(opt.exp_dir, exp_name)
    ...
    if os.path.exists(os.path.join(exp_path, "memory_pool.pth")) == True:
        print("memory pool has already existed. If you want to re-generate it, please delete the existing file.")
    else:
        for iteration, viewpoint_cam in enumerate(viewpoint_stack):
            ...
            pose, render_semantics, render_depth, gt_image, gt_semantic_map,tips1_coord, tips2_coord = track(instrument, pose, viewpoint_cam, opt, pipe, background, iteration)
